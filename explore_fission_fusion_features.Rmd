...
events$fusion.type.sym[grepl('move', events$fusion.type) & grepl('stay', events$fusion.type)] <- 'fusion.stay.move'
## rename move.stay and stay.move to be move.stay and move.stay.swap
# stay.move -> move.stay = move.stay.swap
events$fission.type.sym[grepl('stay\\.move', events$fusion.type) & grepl('move\\.stay', events$fission.type)] <- 'fission.move.stay'
#stay.move -> stay.move = stay.move (no change)
#move.stay -> move.stay = stay.move
events$fission.type.sym[grepl('move\\.stay', events$fusion.type) & grepl('move\\.stay', events$fission.type)] <- 'fission.stay.move'
#move.stay -> stay.move = stay.move.swap
events$fission.type.sym[grepl('move\\.stay', events$fusion.type) & grepl('stay\\.move', events$fission.type)] <- 'fission.move.stay'
#move.move (both types) -> stay.move or move.stay = stay.move 
events$fission.type.sym[grepl('move\\.move', events$fusion.type) & grepl('stay', events$fission.type)] <- 'fission.stay.move'
events$fission.type.sym[which(events$fission.type.sym == 'fission.stay.move' & is.na(events$fusion.type.sym))] <- 'fission.stay.move'
...
trans.travel <- table(events$fission.type[idxs.travel], events$fusion.type[idxs.travel])
...
# trans.travel.na['fission.move.stay', 'fusion.move.move.aligned'] <- NA
# trans.travel.na['fission.move.stay', 'fusion.move.move.unaligned'] <- NA
# trans.local.na <- trans.local
# trans.local.na['fission.move.stay', 'fusion.move.move.aligned'] <- NA
# trans.local.na['fission.move.stay', 'fusion.move.move.unaligned'] <- NA
...
par(mar = c(12,12,2,2))
image.plot(trans.travel, xaxt = 'n', yaxt = 'n', col = viridis(256))
...
chordDiagram(t(trans.local),
             grid.col = c(fusion.cols[c(1,2,4)], fusion.cols), annotationTrack = 'grid', preAllocateTracks = list(list(list(track.height = 0.001, list(track.height = 0.9)))))
...
chordDiagram(t(trans.travel),
             grid.col = c(fusion.cols[c(1,2,4)], fusion.cols), annotationTrack = 'grid', preAllocateTracks = list(list(list(track.height = 0.3, list(track.height = 0.9)))))
