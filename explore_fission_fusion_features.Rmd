...
![](fission_fusion_event_figure.png)
...
daylight.pal <- colorRampPalette(colors = c(viridis::cividis(2)[1], viridis::cividis(2)[2], viridis::cividis(2)[1]))

hist(hour(timestamps[events$t.start]+3*60*60), breaks = seq(0,24,1), xlab = 'Time of day (hour)', ylab = 'Frequency', col = daylight.pal(24), main = 'Time of day of fission-fusion event starts')
hist(hour(timestamps[events$t.end]+3*60*60), breaks = seq(0,24,1), xlab = 'Time of day (hour)', ylab = 'Frequency', col = daylight.pal(24), main = 'Time of day of fission-fusion event ends')
...
Note here that there are a few equivalent event types that we need to combine - for instance fusion.stay.move__together.local__fission.move.stay is equivalent to fusion.move.stay__together.local__fission.stay.move, just with the identities of the individuals swapped. To avoid confusion, let's call the non-symmetrized version the "event.type" and the one in which the symmetries have been collapsed the "event.type.collapsed"
...
#### Elis solution for the transition heatmaps + chord diagrams
events$fusion.type.sym <- events$fusion.type
events$fission.type.sym <- events$fission.type

## combine move.stay and stay.move into one category for fusion events
events$fusion.type.sym[grepl('move', events$fusion.type) & grepl('stay', events$fusion.type)] <- 'fusion.move.stay'
## rename move.stay and stay.move to be move.stay and move.stay.swap
# move.stay -> stay.move = move.stay.swap
events$fission.type.sym[grepl('move\\.stay', events$fusion.type) & grepl('stay\\.move', events$fission.type)] <- 'fission.move.stay.swap'
#move.stay -> move.stay = move.stay (no change)
#stay.move -> stay.move = move.stay
events$fission.type.sym[grepl('stay\\.move', events$fusion.type) & grepl('stay\\.move', events$fission.type)] <- 'fission.move.stay'
#stay.move -> move.stay = move.stay.swap
events$fission.type.sym[grepl('stay\\.move', events$fusion.type) & grepl('move\\.stay', events$fission.type)] <- 'fission.move.stay.swap'
#move.move (both types) -> stay.move or move.stay = move.stay 
events$fission.type.sym[grepl('move\\.move', events$fusion.type) & grepl('stay\\.move', events$fission.type)] <- 'fission.move.stay'
events$fission.type.sym[which(events$fission.type.sym == 'fission.stay.move' & is.na(events$fusion.type.sym))] <- 'fission.move.stay'
...
trans.travel <- table(events$fission.type.sym[idxs.travel], events$fusion.type.sym[idxs.travel])
trans.local <- table(events$fission.type.sym[idxs.local], events$fusion.type.sym[idxs.local])

## Add NA to impossible squares
trans.travel.na <- trans.travel
trans.travel.na['fission.move.stay.swap', 'fusion.move.move.aligned'] <- NA
trans.travel.na['fission.move.stay.swap', 'fusion.move.move.unaligned'] <- NA
trans.local.na <- trans.local
trans.local.na['fission.move.stay.swap', 'fusion.move.move.aligned'] <- NA
trans.local.na['fission.move.stay.swap', 'fusion.move.move.unaligned'] <- NA

par(mar = c(12,12,2,2))
image.plot(trans.local.na, xaxt = 'n', yaxt = 'n', col = viridis(256))
...
par(mar = c(12,12,2,2))
image.plot(trans.travel.na, xaxt = 'n', yaxt = 'n', col = viridis(256))
axis(side = 1, at = seq(0,1, length.out = nrow(trans.travel)), rownames(trans.travel), las = 2)
axis(side = 2, at = seq(0,1, length.out = ncol(trans.travel)), colnames(trans.travel), las = 2)
title(main = 'Transitions for traveling events')



## Chord diagrams 
par(mfrow = c(1,1), par(mar = c(1,1,1,1)))
circos.par(start.degree = 270)
chordDiagram(t(trans.local),
             grid.col = c(fusion.cols[1:3], fusion.cols), annotationTrack = 'grid', preAllocateTracks = list(list(list(track.height = 0.001, list(track.height = 0.9)))))

circos.track(track.index = 2, panel.fun = function(x, y) {
    circos.text(CELL_META$xcenter, CELL_META$ylim[1], gsub('\\.', '\n', gsub('fusion|fission', '', CELL_META$sector.index)), 
                facing = 'inside', niceFacing = TRUE)
}, bg.border = NA)
highlight.sector(rownames(trans.local), track.index = 1, col = alpha("white", 0), 
    text = "Fission", cex = 1.1, text.col = "black", niceFacing = F)
highlight.sector(colnames(trans.local), track.index = 1, col = alpha("white", 0), 
    text = "Fusion", cex = 1.1, text.col = "black", niceFacing = F)
...
chordDiagram(t(trans.travel),
             grid.col = c(fusion.cols[1:3], fusion.cols), annotationTrack = 'grid', preAllocateTracks = list(list(list(track.height = 0.3, list(track.height = 0.9)))))

circos.track(track.index = 2, panel.fun = function(x, y) {
    circos.text(CELL_META$xcenter, CELL_META$ylim[1], gsub('\\.', '\n', gsub('fusion.|fission.', '', CELL_META$sector.index)), 
                facing = 'inside', niceFacing = TRUE)
}, bg.border = NA)
highlight.sector(rownames(trans.travel), track.index = 1, col = alpha("white", 0), 
    text = "Fission", cex = 1.1, text.col = "black", niceFacing = F)
highlight.sector(colnames(trans.travel), track.index = 1, col = alpha("white", 0), 
    text = "Fusion", cex = 1.1, text.col = "black", niceFacing = F)
abline(v = 0, lty = 2, col = "#00000080")
title('Travel')
circos.clear()
...
```{r animation function, echo = FALSE}
animate_ff_event <- function(event.row, events = events, xs = xs, ys = ys, frame.step = 20, tail.step = 5, tail = 300){
  ind.i <- events$i[event.row]
  ind.j <- events$j[event.row]
  t0 <- events$t.start[event.row]
  tf <- events$t.end[event.row]
  b1 <- events$b1[event.row]
  b2 <- events$b2[event.row]
  t.idxs <- seq(t0, tf, by = frame.step)
  xmax <- max(max(xs[ind.i,t.idxs], na.rm=T),max(xs[ind.j,t.idxs], na.rm=T))
  xmin <- min(min(xs[ind.i,t.idxs], na.rm=T),min(xs[ind.j,t.idxs], na.rm=T))
  ymax <- max(max(ys[ind.i,t.idxs], na.rm=T),max(ys[ind.j,t.idxs], na.rm=T))
  ymin <- min(min(ys[ind.i,t.idxs], na.rm=T),min(ys[ind.j,t.idxs], na.rm=T))
  col1 <- 'blue'
  col2 <- 'red'
  for(delay in 1:10){
    par(bg = 'black')
    plot(NULL, xlim = c(xmin,xmax), ylim = c(ymin, ymax), 
         asp = 1, bg = 'black', xlab = '', ylab = '', main = events$event.type[event.row])
  }
  for (t in t.idxs) {
    ts <- seq(t - tail, t, by = tail.step)
    xi <- xs[ind.i, ts] - xmin
    yi <- ys[ind.i, ts] - ymin
    xj <- xs[ind.j, ts] - xmin
    yj <- ys[ind.j, ts] - ymin
    cols1 <- rep('darkblue', length(ts))
    cols1[which(ts > b1)] <- 'blue'
    cols1[which(ts > b2)] <- 'cyan'
    cols2 <- rep('darkred', length(ts))
    cols2[which(ts > b1)] <- 'red'
    cols2[which(ts > b2)] <- 'orange'
    par(bg = 'white')
    plot(NULL, xlim = c(0,xmax - xmin), ylim = c(0, ymax - ymin), asp = 1, xlab = '', ylab = '', main = events$event.type[event.row])
    sizes <- seq(.1,1,length.out = length(xi))
    points(xi, yi, pch = 19, cex = sizes, col = scales::alpha(cols1, 0.5))
    points(xj, yj, pch = 19, cex = sizes, col = scales::alpha(cols2, 0.5))
    legend('topright',legend = paste(round((t - min(t.idxs))/60), 'min'))
  }
}
```
=======
