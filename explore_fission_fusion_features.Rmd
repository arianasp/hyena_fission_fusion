...
chordDiagram(t(trans.local),
             grid.col = rep(fusion.cols, 2), annotationTrack = 'grid', preAllocateTracks = list(list(list(track.height = 0.001, list(track.height = 0.9)))))
...
animate_event <- function(event.row, events = events, xs = xs, ys = ys, frame.step = 20, tail.step = 5, tail = 300){
...
The most common type is fusion.move.move.unaligned__together.local__fission.move.move.unaligned - there are 55 events of this type. We call this type "crossing".

```{r, animation.hook="gifski", interval = 0.1, echo = FALSE}
#crossing
row <- 30
animate_event(event.row = row, events = events, xs = xs, ys = ys)
```

Next most common is fusion.stay.move__together.local__fission.stay.move - we interpret this as "passing" (51 events). We call this type "passing".
...
Next up is fusion.stay.move__together.local__fission.move.stay (45 events). We call this "displacement".
