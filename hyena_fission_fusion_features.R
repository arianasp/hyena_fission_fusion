...
#FEATURES OF THE TOGETHER PHASE

#movement coordination is defined as the mean heading correlation between two individuals over a period of time, only counting instances
#where both individuals moved a minimum displacement min.disp. Speeds are computed as the displacement over a period of heading.win seconds
#divided by heading.win

heading.win <- 30 
min.disp <- 5
synch.thresh <- 0.7

together.seqs$heading.corr.together <- NA
for(i in 1:nrow(together.seqs)){
  
  #if together phase is undefined, skip
  if(is.na(together.seqs$b1[i]) | is.na(together.seqs$b2[i])){
    next
  }
  
  if((together.seqs$b2[i] - together.seqs$b1[i]) <= heading.win){
    next
  }
  #get x and y coordinates of i and j during together phase
  xi <- xs[together.seqs$i[i], together.seqs$b1[i]:together.seqs$b2[i]]
  yi <- ys[together.seqs$i[i], together.seqs$b1[i]:together.seqs$b2[i]]
  xj <- xs[together.seqs$j[i], together.seqs$b1[i]:together.seqs$b2[i]]
  yj <- ys[together.seqs$j[i], together.seqs$b1[i]:together.seqs$b2[i]]  
  
  dt.tot <- length(xi)
  
  #get displacements over time
  dx.i <- xi[(1+heading.win):dt.tot] - xi[1:(dt.tot - heading.win)]
  dy.i <- yi[(1+heading.win):dt.tot] - yi[1:(dt.tot - heading.win)]
  ds.i <- sqrt(dx.i^2 + dy.i^2)
  dx.j <- xj[(1+heading.win):dt.tot] - xj[1:(dt.tot - heading.win)]
  dy.j <- yj[(1+heading.win):dt.tot] - yj[1:(dt.tot - heading.win)]
  ds.j <- sqrt(dx.j^2 + dy.j^2)
  
  #get headings over time
  heads.i <- atan2(dy.i, dx.i)
  heads.j <- atan2(dy.j, dx.j)
  
  #get heading correlation over time
  head.cor <- (dx.i * dx.j + dy.i * dy.j) / (ds.i * ds.j)
  head.cor.sub <- head.cor
  head.cor.sub[which(ds.i < min.disp | ds.j < min.disp)] <- 0
  
  together.seqs$heading.corr.together[i] <- mean(head.cor.sub, na.rm=T)
}

hist(together.seqs$heading.corr.together, breaks = 100)
